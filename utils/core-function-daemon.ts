 ...
 *
 * 原理：
 * 再等待 option.delay 毫秒后，每隔 option.intervalTime 毫秒，执行 dataValidator 检查参数 value 的值是否合法。
 * 如果它的值不合法，则执行 requestDataFn 重新获取数据，并将计数器加一。
 * 重复该步骤，直到：
 *   1. 计数器数量大于等于 option.attempts，即达到尝试次数上限。
 *   2. 被检查的 Vue 响应式变量的值不为 undefined.
 *
 * @param value 被检查的变量。
...
export function observeEmptyVarbAndRequestData<T>(
	...
	startTimer = setTimeout(() => {
		intervalTimer = setInterval(async () => {
			...
			if (count >= option.attempts || !dataValidator(value))
				if (intervalTimer) {
